...
import { BellIcon, ClockIcon, MessageSquareIcon, RefreshCcw, RefreshCcwDot, RefreshCwIcon, UserCheckIcon } from "lucide-react";
import NoNotificationsFound from "../components/NoNotificationsFound";
import {formatDistanceToNow} from "date-fns"
const NotificationsPage = () => {
  const queryClient = useQueryClient();

  const { data: friendRequests, isLoading, refetch ,isFetching } = useQuery({
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div>
            <button onClick={()=>refetch()} disabled={isFetching}>
              <RefreshCwIcon className={`size-5 hidden sm:inline ${ isFetching?(`animate-spin`):(`animate-none`)} `}/>
            </button>
          </div>
